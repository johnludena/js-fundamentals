...
var getFullNames = function(objArray) {
    ...
    for (var i = 0; i < objArray.length; i++) {
        var firstName = objArray[i].first
        var lastName = objArray[i].last
...
var generateDog = function(nameProp) {
    var newDog = {
        name: nameProp,
        legs: 4,
        weight: 60,
...
var filterString = function(input) {
    var re = /[.,’“”\/#!$%\^&\*;:{}=\-_`~()]?/g
    var filteredString = input.replace(re,"")
...
var getCounts = function(string) {
    ...
    var lowerCaseString = toLowerCase(string)
    var cleanString = filterString(lowerCaseString)
